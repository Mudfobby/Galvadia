...
//stasis commands
const handleCombatStasisInputs = function (commandLineInput) {
	...
	if (player.advancing == true) {
		quickMessage(`You are currently engaging your target and cannot perform other actions for [${advanceTimer}] seconds`)
	} else if (player.retreating == true) {
		quickMessage(`You are currently retreating and cannot perform other actions for [${retreatTimer}] seconds`)
	} else {
		switch (command) {
			case 'pull':
				pull(secondCommand)
...
function advance() {
	...
	if (targetedMonster) {
		...
		advanceCount(player.advanceSpeed)
		quickMessage(`You engage the ${targetedMonster.name} - [${player.advanceSpeed}] seconds until you can act`)

		setTimeout(() => {
			player.stasis = false
			player.advancing = false
		}, player.advanceSpeed * 1000)
	} else {
...
function retreat(secondCommand) {
	...
	if (secondCommand === retreatDirection && secondCommand !== undefined) {
		...
		retreatCount(player.retreatSpeed)
		quickMessage(`You quickly retreat to the ${secondCommand} - [${player.retreatSpeed}] seconds until you can act`)

		setTimeout(() => {
			player.stasis = false
			player.retreating = false
		}, player.retreatSpeed * 1000)
	} else {
		player.stasis = true
		player.retreating = true

		retreatCount(player.retreatSpeed)
		quickMessage(`You disengage and retreat from the monster - [${player.retreatSpeed}] seconds until you can act.`)
...
function monsterToNode(monster) {
	...
	monsterSpanNode.classList.add(monster.color)
...
function attack(secondCommand) {
	...
	function monsterTarget(targetedMonster, damage1, damage2) {
		...
		if (targetedMonster.health <= 0) {
			...
			console.log(targetedMonster.death()) //this function call displays all the dead monsters death text - exp gained, gold dropped, items dropped, death message
...
function damageCalculator(playerWeapon) {
	...
	console.log(targetedMonster.dodge, ' TARGETED MONSTER DODGE CHANCE')
	console.log(player[playerWeaponSkill].accuracy, ' TARGETED PLAYER WEAPONSKILL ACCURACY')
...
function playerLevelFunc() {
	...
	if (playerExp <= 69) {
		...
		player.percentToNextLevel = Math.floor((player.experience / player.experienceNeededToLevel) * 1000) / 10
...
function updateOneHandedAccuracy() {
	player.oneHanded.accuracy = player.str + player.dex + player.oneHanded.level + 100
}
function updateOneHandedAttackSpeed() {
	player.oneHanded.speed = 4 - Math.floor(player.oneHanded.level / 10)
}
...
function updateTwoHandedAccuracy() {
	player.twoHanded.accuracy = player.str + player.agi + player.twoHanded.level + 100
}
function updateTwoHandedAttackSpeed() {
	player.twoHanded.speed = 6 - Math.floor(player.twoHanded.level / 10)
}
...
function updateUnarmedAttackPower() {
	...
	player.unarmed.attackPower = player.str + player.dex + player.agi + player.unarmed.level - 1
}
function updateUnarmedAccuracy() {
	player.unarmed.accuracy = player.agi + player.dex + player.unarmed.level + 100
}
function updateUnarmedAttackSpeed() {
	player.unarmed.speed = 4 - Math.floor(player.unarmed.level / 10)
}
...
centralBeginnersRoom.roomDialogue = `Using the 'look' command will refresh your view of the room as if you had just entered it. This will display any items in the room that might have been dropped by a monster after killing it. You can also look into other connected rooms before entering by typing 'look' and then the direction you want to look (ex: 'look northwest'). This can be useful in dangerous areas if you want to check if there are any monsters in the room before entering.`
		...
		if (doKeywordsMatch == true) {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					dialogue(currentArea.roomDialogue)
					setTimeout(() => {
						dialogue(`For your next task, head to the northwest.`)
					}, 0)
				}, 0)
				player.stasis = false
			}, 1000)
...
let northwestBeginnersRoom = new AreaMaker(
	...
	{
		lever: {
			names: ['lever', 'test lever'],
			desc: function () {
				console.log(currentArea)
				if (currentArea.descriptions.zoneExitsBool.southeast == undefined) {
					quickMessage(`After pulling the lever, you hear a rumbling to the southeast`)
					currentArea.descriptions.zoneExitsBool.southeast = true
					currentArea.descriptions.zoneExitsFunc()
				} else if (currentArea.descriptions.zoneExitsBool.southeast == true) {
					quickMessage(`You pull the lever again. Nothing happens.`)
				}
			},
		},
	}
...
function pull(secondCommand) {
	...
	if (objectBeingPulled != undefined) {
		objectBeingPulled.desc()
...
northwestBeginnersRoom.roomDialogue = `The directions you can travel will always be shown beside 'Exits:'. However, there are secrets scattered around the world. When discovered, some of these secrets might open up a hidden path. Most secrets that open up a new path are discovered by inspecting objects from the room description. You'll get a chance to discover one of those areas soon enough. For now, go back to the Central Training Room.`
...
let northBeginnersRoom = new AreaMaker(
	...
	(descriptions = {
		...
		desc: `Type REPEAT to hear the explaination about inventory again.`,
		zoneExitsBool: {
			south: true,
		},
		zoneExits: [],
...
northBeginnersRoom.roomDialogue = `To see all the items that you own, check your INVENTORY (or 'i'). You currently do not have any items on you.`
...
northeastBeginnersRoom.roomDialogue = `Use the EXAMINE command`
	...
	for (let x in currentArea.questKeywords) {
		...
		if (player.stasis == false && currentArea.questKeywords != undefined) {
			if (questKeyword.find(keyword => keyword == questWord) == questWord && currentArea.isQuestComplete1 == false) {
				...
				player.stasis = true
				setTimeout(() => {
					dialogue(`It's time to move on to some more interesting topics! You will now be on your own and must explore your surroundings to find your way. You can always return to these rooms if you forgot anything. The REPEAT command will repeat a room's tutorial information. `)
					player.stasis = false
				}, 1000)
			}
		}
...
let trainingHallMessRoom = new AreaMaker( //change name
	true,
	[],
	false,
	areaIdGenerator(),
	...
	(descriptions = {
		areaNameClass: castleGrey,
		areaName: `Training Hall Mess Room`, //change area name
		desc: `You stand in a large, crowded mess hall. Guild prospects are finishing their meals, and milling about the room before getting back to their training. A burly man stands guard at the western door`, //`CENTRAL TRAINING ROOM - new recruits practicing their stances and strikes. A burly guard stands at the western door`, //change area desc
		zoneExitsBool: {
			north: true,
			east: true,
...
//change room name
trainingHallMessRoom.questKeywords = {
	//change quest keywords
	quest1: ['west', 'w'],
}
//change room name
trainingHallMessRoom.isQuestComplete1 = false
//change room name
trainingHallMessRoom.questComplete = function (questWord) {
	let questKeyword
	for (let x in currentArea.questKeywords) {
		if (currentArea.questKeywords[x].find(x => x == questWord)) {
			questKeyword = currentArea.questKeywords[x]
		...
		if (player.stasis == false && currentArea.questKeywords != undefined) {
			if (questKeyword.find(keyword => keyword == questWord) == questWord && currentArea.isQuestComplete1 == false) {
				...
				player.stasis = true
				setTimeout(() => {
					dialogue(`Guard Tulaf says, "You cannot leave until you've finished your training."`)
					player.stasis = false
				}, 1000)
			}
		}
...
let combatHalls1 = new AreaMaker( //change name
	true,
	[],
	false,
	areaIdGenerator(),
	...
	(descriptions = {
		areaNameClass: castleGrey,
		areaName: `Long hallway`, //change area name
		desc: `You stand at the beginning of a very long hallway. Doors on each side line the hall. Each door will lead you to a room that will teach you about combat.`,
		zoneExitsBool: {
			north: true,
			east: true,
			south: true,
...
let combatRoom1 = new AreaMaker( //change name
	true,
	[],
	false,
	areaIdGenerator(),
	...
	(descriptions = {
		areaNameClass: castleGrey,
		areaName: `Room 1 - COMBAT, ADVANCING, RETREATING, `, //change area name
		desc: `Monsters with a yellow name are not hostile and will not look to attack you. However, if you decided to engage combat with them, they may fight back. Inspecting an enemy might give you a better idea of their danger. Once the training dummy has been defeated, you will be able to pick up the items that he drops. `,
		zoneExitsBool: {
			east: true,
		},
		zoneExits: [],
...
function trainingOnehandedSword(monsterLevelOrShopValue, isMonsterDrop) {
	//This funciton is executed when either a monster dies and drops loot, or when purchased from a shop
	let monsterLevel = monsterLevelOrShopValue //this is the level of the monster that drops this item
	let monsterSellValue = monsterLevel * 0.5 + 8 //this is the value of the item when obtained from a monster drop
	let vendorBuyValue = monsterLevelOrShopValue //this is the pre-set BUY price of the item
	let vendorSellValue = 1 + monsterLevelOrShopValue * 0.5 //this is the SELLBACK value - only this when purchased from a shop
	let trainingOnehandedSword = {
		id: () => {
			for (let i = 0; i < 50; i++) {
				if (!pushItem[i]) {
					return i
		...
		keywords: ['training sword', 'sword'],
		topDamage: randomNumberRange(1, 1),
		bottomDamage: randomNumberRange(1, 1),
		mods: {
			str: 0,
			con: 0,
			slashingPen: 1,
		...
		sellValue: isMonsterDrop == true ? monsterSellValue : vendorSellValue,
		desc: (itemId, bottomDamage, topDamage, itemName, mods) => {
			const description = `This sword is only meant to be used for training. Not for actual combat.`
			weaponDescription(itemId, bottomDamage, topDamage, itemName, mods, description)
		},
		swing: (weapon, monster, damage) => rustedSwordSwing(expertise(weapon), playerSwingType(), playerWeaponName(weapon), atIntoOnto(), enemyName(monster), damageNumber(damage), playerPenetrationNameFunc(playerPenetrationName), damage),
	}
	return trainingOnehandedSword
}
function trainingFist(monsterLevelOrShopValue, isMonsterDrop) {
	//This funciton is executed when either a monster dies and drops loot, or when purchased from a shop
	let monsterSellValue = 1
	let vendorBuyValue = 1
	let vendorSellValue = 1
	let trainingFist = {
		id: () => {
			for (let i = 0; i < 50; i++) {
				if (!pushItem[i]) {
		...
		sellValue: isMonsterDrop == true ? monsterSellValue : vendorSellValue,
		desc: (itemId, bottomDamage, topDamage, itemName, mods) => {
			const description = `This fist weapon is only meant to be used for training. Not for actual combat.`
			weaponDescription(itemId, bottomDamage, topDamage, itemName, mods, description)
		},
		swing: (weapon, monster, damage) => unarmedSwing(expertise(weapon), playerSwingType(), playerWeaponName(weapon), atIntoOnto(), enemyName(monster), damageNumber(damage), playerPenetrationNameFunc(playerPenetrationName), damage),
	}
	return trainingFist
}
function trainingTwohandedSword(monsterLevelOrShopValue, isMonsterDrop) {
	//This funciton is executed when either a monster dies and drops loot, or when purchased from a shop
	let monsterSellValue = 1
	let vendorBuyValue = 1
	let vendorSellValue = 1
	let trainingTwohandedSword = {
		...
		keywords: ['training', 'twohanded', 'sword', 'twohanded sword'],
		topDamage: randomNumberRange(1, 1),
		bottomDamage: randomNumberRange(1, 1),
		mods: {
			str: 0,
		...
		sellValue: isMonsterDrop == true ? monsterSellValue : vendorSellValue,
		desc: (itemId, bottomDamage, topDamage, itemName, mods) => {
			const description = `This twohanded sword is only meant to be used for training. Not for actual combat.`
			weaponDescription(itemId, bottomDamage, topDamage, itemName, mods, description)
		},
		swing: (weapon, monster, damage) => rustedSwordSwing(expertise(weapon), playerSwingType(), playerWeaponName(weapon), atIntoOnto(), enemyName(monster), damageNumber(damage), playerPenetrationNameFunc(playerPenetrationName), damage),
	}
...
function trainingDummy(area) {
	let trainingDummy = {
		...
		death: () => {
			quickMessage(`The training dummy explodes into a million pieces!`)
			quickMessage(`You gain [${this.experience}] experience points`)
			quickMessage(`The training dummy has dropped...`)
			quickMessage(`${this.gold} gold piece(s)`)
			this.itemDrops.forEach(item => quickMessage(`${item.name}`))
			setTimeout(() => {
				dialogue(`Great job! To pick up items from the ground ......`)
			}, 1000)
		},
	}
...
const inspect = (secondCommand, thirdCommand) => {
	...
	let firstMonsterInRoom = pushMonster.find(monster => currentArea.id == monster.roomId)
	let roomInteractable = currentArea.interactables != undefined && currentArea.interactables[secondCommand] != undefined ? currentArea.interactables[secondCommand] : undefined
	//inspects object from the room description
	if (roomInteractable != undefined && roomInteractable.names.find(name => name == secondCommand) != undefined) {
	...
	//inspects NPC
		...
		npc.desc()
	} else if (firstMonsterInRoom != undefined && firstMonsterInRoom.name == secondCommand) {
...
*/
currentArea = combatRoom1

monsterGen(testBlob(currentArea))
monsterGen(trainingDummy(currentArea))
weaponGen(trainingFist(currentArea))
weaponGen(trainingOnehandedSword(currentArea))
weaponGen(trainingTwohandedSword(currentArea))
function gameStart() {
	areaCompiler(currentArea)
	player.stasis = true
	setTimeout(() => {
